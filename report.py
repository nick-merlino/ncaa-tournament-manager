...
from constants import ROUND_ORDER, ROUND_WEIGHTS  # Import round weights from constants
...
def generate_report(pdf_filename=None):
    ...
    The report includes:
      ...
      - Visual sections:
          ...
          * Upsets table (all upsets, unlimited).
          * Bar charts for the 10 most popular and 10 least popular teams still remaining.
    ...
    try:
        ...
        # Build DataFrame: each row corresponds to one user pick with associated user score.
        data_rows = []
        ...
        # Determine current round and round game details from tournament results.
        ...
        # ROUND_WEIGHTS is now imported from constants.py.
        # Build winners_by_round mapping (using base round names)
        winners_by_round = {}
        ...
        if upsets:
            ...
            # List all upsets without limiting the number.
...
def determine_team_status(team, current_round, round_games):
    """
    Determines the status of a team based on the tournament progress.
    
    Returns:
      - 'in': if the team has won in the current or previous rounds.
    ...
    Args:
        ...
        round_games (dict): Dictionary mapping round names to game details.
    
    Returns:
        str: 'in', 'out', or 'not_played'
    """
    ROUND_LIST = ROUND_ORDER  # Already defined in constants.py
    current_index = ROUND_LIST.index(current_round)
